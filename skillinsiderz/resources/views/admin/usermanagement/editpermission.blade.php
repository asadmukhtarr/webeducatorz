            ...
            <table class="table table-bordered">
                ...
                <tr>
                    <form action="{{ route('update.permission',$permission->id) }}" method="post">
                        @csrf
                        <td>
                            <input type="text" value="{{ $permission->name }}" class="form-control" placeholder="Title" name="title" /> 
                            <font color="red"> 
                                <b> @error('title') <span class="error">{{ $message }}</span> @enderror </b>
                            </font>
                        </td>
                        <td>
                            <input type="text" value="{{ $permission->slug }}"  class="form-control" placeholder="Slug" name="slug" /> 
                            <font color="red"> 
                                <b> @error('slug') <span class="error">{{ $message }}</span> @enderror </b>
                            </font>
                        </td>
                        <td>
                            <input type="text" value="{{ $permission->route_name }}"  class="form-control" placeholder="Route" name="route_name" />
                            <font color="red"> 
